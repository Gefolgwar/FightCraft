import { getTemplates, saveTemplate, deleteTemplate, clearLocationObjects, saveGeneratedObjects, saveWorldSnapshot, getWorldSnapshots, getSnapshotById, isAdmin, getCurrentUser, initFirebase } from './firebase-service.js';
...
window.loadWorldSnapshots = async function () {
    ...
    try {
        ...
        snaps.forEach(snap => {
            ...
            const typeIcon = snap.type === 'shop' ? '🏪' : (snap.type === 'monster' ? '👾' : '🌍');
...
window.startGeneration = async (overwrite = true) => {
    ...
    // Mode Logging
    logConsole(overwrite ? "🗑️ Mode: Create / Delete + Create (Replenish Shops)" : "➕ Mode: Add if exist");

    // 1. Initial State & Preservation
    let existingObjects = [];
    let zonesGeoJson = null;
    const { getSnapshotById } = await import('./firebase-service.js');
    ...
    if (targetTemplateId === 'new') {
        if (window.autoTargetSnapName) {
            ...
            snapName = prompt("Enter a name for this new Map Template:", `${cityKey}_shops_${new Date().toLocaleDateString()}`);
            if (!snapName) return;
        }
    } else {
        const selector = document.getElementById('gen-template');
    ...
    if (citadels.length >= 2 && !zonesGeoJson) {
        ...
        try {
            const { generateCityTerritory } = await import('./territory-service.js');
    ...
    logConsole(`📡 Connecting to Overpass API for ${city.name} (searching 10km area, targeting ${capacity} shops)...`);
    ...
    try {
        ...
        if (zonesGeoJson && zonesGeoJson.features.length > 0) {
            ...
            logConsole(`⚖️ Distributing ${capacity} shops across ${totalLogicalZones} logical zones...`);
        ...
        if (success) {
            ...
            if (zonesGeoJson) {
                try {
                    const { saveCityZones } = await import('./firebase-service.js');
