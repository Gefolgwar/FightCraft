...
import { getDatabase, ref, set, push, update, remove, onChildAdded, onChildChanged, runTransaction, onValue, off, onDisconnect, query as rtdbQuery, orderByChild, equalTo, serverTimestamp as rtdbTimestamp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-database.js";
...
import { monitoredGetDoc, monitoredGetDocs, monitoredOnSnapshot } from './firebase-monitor.js';
...
import { showNotification, addEventLog } from '../ui/ui-controller.js';
import { SyncEngine } from './sync-engine.js?v=debug_11';
...
const firebaseConfig = {
    ...
    databaseURL: "https://fight-craft-3c3f0-default-rtdb.europe-west1.firebasedatabase.app/"
};
            ...
            const unsubscribe = onAuthStateChanged(auth, async (user) => {
                ...
                if (user) {
                    ...
                    try {
                        ...
                        const cachedProfileRaw = localStorage.getItem(`user_profile_${user.uid}`);
                        ...
                        if (cachedProfile) {
                            ...
                            if (userRole === 'admin') showNotification('👑 Cache: Welcome, Admin!', 'success');
                        } else {
                            // Fetch from Firestore
                            console.log('⏳ Fetching User Profile...');
                            // trackUsage passed to monitoredGetDoc
                            const userRef = doc(db, 'users', user.uid);
                            const userDoc = await monitoredGetDoc(userRef, `users/${user.uid} (initFirebase Profile)`);

                            if (userDoc.exists()) {
                                const data = userDoc.data();
                                userRole = data.role || 'player';
                                if (userRole === 'admin') showNotification('👑 Welcome back, Admin!', 'success');

                                // Update Cache
                                const cacheObj = { data: data, timestamp: Date.now() };
                                localStorage.setItem(`user_profile_${user.uid}`, JSON.stringify(cacheObj));
                                ...
                                if (Object.keys(updates).length > 0) {
                                    ...
                                    updateDoc(userRef, updates).catch(e => console.warn("Update profile failed:", e));
                                }
                            } else {
                    ...
                    console.log(`ℹ️ Firebase: No user detected in ${duration}s. Redirecting...`);
                    window.location.href = 'login.html';
                    resolve(false);
                }
            });
...
export async function savePlayerToCloud(playerData) {
    ...
    try {
        ...
        trackUsage('write', '[sync] [збереження даних профілю]', 1, `users/${currentUser.uid} (savePlayerToCloud)`, playerData);
...
export async function loadPlayerFromCloud() {
    ...
    try {
        ...
        const docSnap = await monitoredGetDoc(userRef, `users/${currentUser.uid} (loadPlayerFromCloud)`);

        if (docSnap.exists()) {
            console.log("Cloud data found:", docSnap.data());

            // Update cache while we are here
            localStorage.setItem(`user_profile_${currentUser.uid}`, JSON.stringify({
                data: docSnap.data(),
                timestamp: Date.now()
            }));
...
export async function updatePlayerLocation(lat, lng) {
    ...
    try {
        ...
        const userRef = doc(db, "users", currentUser.uid);
        trackUsage('write', '[gps] [оновлення координат (Firestore)]', 1, `users/${currentUser.uid}/position (updatePlayerLocation)`, { lat, lng });
...
export function subscribeToPlayers(onUpdate) {
    ...
    console.warn("⚠️ subscribeToPlayers (Firestore) called! This uses collectionGroup (expensive). Call Stack:", new Error().stack);
    ...
    try {
        ...
        trackUsage('read', '[multiplayer] [пошук активних гравців]', 1, 'characters/ (subscribeToPlayers)');
        const unsubscribe = monitoredOnSnapshot(q, (snapshot) => {
            ...
            if (error.code === 'permission-denied') {
                console.log("Multiplayer Sync: Access Denied (Admin-only feature). Player list hidden.");
...
function subscribeToPlayersLegacy(onUpdate) {
    ...
    trackUsage('read', '[multiplayer] [пошук гравців (Legacy)]', 1, 'users/ (subscribeToPlayersLegacy)');
    const unsubscribe = onSnapshot(q, (snapshot) => {
        trackUsage('read', '[multiplayer] [оновлення гравців (Legacy)]', snapshot.size, `users/ (Legacy Snapshot Update)`);
        const players = [];
        snapshot.forEach((doc) => {
            const data = doc.data();
            const name = data.player?.name || 'Unknown';
...
export async function getAllPlayersForDebug() {
    ...
    try {
        ...
        if (error.code === 'permission-denied') {
            console.warn("Debug character fetch failed - likely missing composite index or quota.");
        }
        return [];
    }
}
...
export async function loadTestPlayersToMap() {
    try {
        ...
        const { otherPlayerMarkers, createPlayerMarker } = await import('../map/map.js');
...
export async function getCharacter(userId, characterId) {
    ...
    try {
        ...
        console.log(`🔍 getCharacter: Requesting users/${userId}/characters/${characterId}`);
        console.log(`   👉 Auth Context: CurrentUID=${currentAuthId} vs ParamUserID=${userId}`);

        if (currentAuthId !== userId) {
            console.warn("⚠️ getCharacter: ID Mismatch! This may cause Permission Denied.");
        }
        ...
        if (docSnap.exists()) {
            ...
            console.warn(`❌ Character not found: ${characterId} (Path: users/${userId}/characters/${characterId})`);
            return null;
        }
...
export async function saveCharacter(userId, characterId, characterData) {
    ...
    try {
        ...
        trackUsage('write', '[sync] [збереження персонажа]', 1, `users/${userId}/characters/${characterId} (saveCharacter)`, cleanData);
...
export async function saveTemplate(template) {
    ...
    try {
        ...
        trackUsage('write', `[admin] [збереження шаблону: ${template.type}]`, 1, `templates/${id}`, data);
...
export async function deleteTemplate(templateId) {
    if (!isAdmin()) return false;
    try {
        const { doc, deleteDoc, setDoc, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
        trackUsage('delete', '[admin] [видалення шаблону]', 1, `templates/${templateId}`);
...
export async function clearLocationObjects(cityId, type) {
    ...
    try {
        ...
        trackUsage('delete', `[admin] [очищення локації: ${type} в ${cityId}]`, count, 'spawned_objects/');
        ...
        _spawnedObjectsCache = _spawnedObjectsCache.filter(o => !idsToDelete.has(o.id));

        // Clear Packed State (force re-pack on next load)
        const packedRef = doc(db, 'world_chunks', cityId);
        deleteDoc(packedRef).catch(err => console.error('Failed to clear packed:', err));

        console.log(`✅ Cleared ${count} old objects and updated cache/packed state.`);
        return true;
    } catch (e) {
...
export async function saveGeneratedObjects(objects) {
    ...
    try {
        ...
        trackUsage('write', `[admin] [масове збереження об'єктів]`, count, 'spawned_objects/');
        console.log(`📦 Committing ${chunks.length} batches...`);
        await Promise.all(chunks.map(b => b.commit()));

        // Write-Through Cache: Since IDs are auto-generated and we don't have them yet 
        // without complex logic, we clear the cache to force a fresh fetch if needed.
        // HOWEVER, to be truly optimized, we should ideally NOT clear it.
        // But since these are Admin actions (rare), clearing is a safe middle ground.
...
export function subscribeToSpawnedObjects(cityId, onUpdate) {
    ...
    const q = query(collection(db, 'spawned_objects'), where('cityId', '==', cityId));

    return onSnapshot(q, (snapshot) => {
        trackUsage('read', '[world] [оновлення об\'єктів світу (монстри/замки)]', snapshot.size, `spawned_objects/${cityId}`);
        const objects = [];
        snapshot.forEach(doc => {
...
export async function claimCastle(castleId, castleData) {
    ...
    try {
        const { doc, setDoc, serverTimestamp, updateDoc } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
        ...
        trackUsage('write', `[world] [захоплення замку: ${castleData.name}]`, 1, `castles/${cid}`, castleData);
        ...
        trackUsage('write', `[world] [оновлення власника замку на мапі]`, 1, `spawned_objects/${cid}`, updateData);
        ...
        if (cachedObj) {
            ...
            }, { merge: true }).catch(err => console.error('Failed to update packed state:', err));
        }
...
export async function abandonCastle(castleId) {
    ...
    try {
        ...
        trackUsage('write', `[world] [залишення замку: ${cid}]`, 2, `castles/${cid}`);
...
export function subscribeToCastles(onUpdate) {
    ...
    try {
        const { collection, query } = window.firebaseFirestore || {
...
export async function getWorldSnapshots() {
    ...
    if (cachedRaw) {
        ...
        } catch (e) { console.warn("Snapshot cache invalid"); }
    }
    ...
    try {
        ...
        trackUsage('read', '[admin] [список знімків світу]', snap.size, 'world_snapshots/');
        ...
        try {
            ...
            const cacheData = results.map(r => ({ ...r, created: r.created.toISOString() }));
            localStorage.setItem(CACHE_KEY, JSON.stringify({ data: cacheData, timestamp: Date.now() }));
        } catch (e) { console.warn("Failed to cache snapshots (quota?)"); }
...
export async function getSnapshotById(id) {
    ...
    try {
        ...
        trackUsage('read', `[admin] [завантаження знімку світу: ${id}]`, 1, `world_snapshots/${id}`);
...
export async function saveCityZones(cityId, geoJson) {
    ...
    try {
        ...
        trackUsage('write', `[admin] [збереження зон міста: ${cityId}]`, 1, `city_zones/${cityId}`, geoJson);
...
export async function fetchNearbyObjects(collectionName, centerLat, centerLng, radiusKm) {
    ...
    try {
        ...
        const snapshot = await getDocs(q);
        trackUsage('read', `[world] [пошук об'єктів поруч (${collectionName})]`, snapshot.size, `${collectionName}/`);
        const results = [];
...
export async function subscribeToInvites(onInvite) {
    if (!db || !currentUser) {
        console.warn("⚔️ PvP: Cannot subscribe to invites: No Auth", { hasDb: !!db, hasUser: !!currentUser });
        return;
    }
    try {
        ...
        const q = query(collection(db, "users", currentUser.uid, "invites"), where("status", "==", "pending"));
        return onSnapshot(q, (snapshot) => {
            console.log(`⚔️ PvP: Invite Snapshot received. Count: ${snapshot.size}`);
            snapshot.docChanges().forEach((change) => {
                if (change.type === "added") {
                    console.log("⚔️ PvP: New invite detected:", change.doc.id, change.doc.data());
...
export async function acceptGroupInvite(invite) {
    if (!db || !currentUser) return;
    try {
        const { doc, updateDoc, setDoc, arrayUnion, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
        ...
        if (!groupId) {
            ...
            const senderCharRef = doc(db, "users", invite.fromUserId, "characters", invite.fromId);
            await updateDoc(senderCharRef, { groupId: groupId });
        } else {
            // Join existing group
            const groupRef = doc(db, "groups", groupId);
            await updateDoc(groupRef, {
                members: arrayUnion({ id: window._currentCharacterId, name: gameState.player.name })
            });
        }

        // 3. Update own character with groupId
        const ownCharRef = doc(db, "users", currentUser.uid, "characters", window._currentCharacterId);
...
export async function sendCombatInvite(targetUserId, targetCharId) {
    ...
    if (!targetUserId) {
        console.error("⚔️ PvP Error: Cannot send invite - targetUserId is missing!");
    ...
    try {
        const { collection, addDoc, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
        const inviteRef = collection(db, "users", targetUserId, "invites");
        console.log(`⚔️ PvP: Sending invite to users/${targetUserId}/invites for char ${targetCharId}`);
...
export async function acceptCombatInvite(invite) {
    if (!db || !currentUser) return null;
    try {
        const { doc, updateDoc, setDoc, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
        ...
        const attackerData = await loadPlayerDataById(invite.fromId, invite.fromUserId);
        ...
        await setDoc(combatRef, {
            ...
            status: 'active',
            participants: [
                { id: invite.fromId, userId: invite.fromUserId, name: invite.fromName, hp: attackerData.hp, maxHp: attackerData.maxHp || 100 },
                { id: window._currentCharacterId, userId: currentUser.uid, name: gameState.player.name, hp: gameState.player.hp, maxHp: gameState.player.maxHp || 100 }
            ],
            logs: [],
 ...
 */
export async function subscribeToInviteStatus(targetUserId, inviteId, onUpdate) {
    ...
    try {
        ...
        if (saved) {
            ...
            if (parsed.date !== today) {
                ...
                usageStats = { reads: 0, writes: 0, deletes: 0, rtdb: 0, logs: [], date: today };
                saveStats();
                return;
            }
...
function saveStats() {
    try {
        if (usageStats.logs.length > 1000) usageStats.logs = usageStats.logs.slice(-1000);
        localStorage.setItem(STORAGE_KEY, JSON.stringify(usageStats));
    } catch (e) { }
}

export function trackUsage(type, description, size = 1, path = 'N/A', data = null) {
...
export async function createBattleRequest(targetUserId, targetCharId) {
    ...
    if (!rtdb) { console.error("❌ RTDB not initialized"); return null; }
    if (!currentUser) { console.error("❌ currentUser missing"); return null; }

    // Check penalty first
    const isPenalized = await checkFleePenalty(currentUser.uid);
    if (isPenalized) {
        showNotification("⏳ You are recovering from fleeing! (5 min cooldown)", "error");
        return null;
    }
    ...
    try {
        ...
        const myCharId = window._currentCharacterId || window._currentlyPlayingCharacterId;
...
export async function applyFleePenalty(uid) {
    ...
    try {
        ...
        console.log(`🏃‍♂️💨 RTDB: Flee penalty applied until ${new Date(penaltyTime).toLocaleTimeString()}`);
    } catch (e) {
...
export async function submitBattleChoice(battleId, choice) {
    ...
    try {
        await runTransaction(requestRef, (currentData) => {
            ...
            if (currentData.choices.attacker === 'fight' && currentData.choices.target === 'fight') {
...
export async function getBattleRequest(battleId) {
    if (!rtdb) return null;
    return new Promise((resolve) => {
        onValue(ref(rtdb, `battle_requests/${battleId}`), (snapshot) => {
...
export async function submitCombatMove(battleId, moveData) {
    ...
    const moveRef = ref(rtdb, `battle_requests/${battleId}/moves/${currentUser.uid}`);
    try {
        await set(moveRef, {
            ...moveData,
 ...
 */
export async function submitRoundResult(battleId, roundResult, nextRoundNumber) {
    if (!rtdb) return;
    const battleRef = ref(rtdb, `battle_requests/${battleId}`);
    try {
    ...
    } catch (e) { console.error("Error submitting round result:", e); }
}
