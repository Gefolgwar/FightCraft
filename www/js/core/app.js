...
import { updateHUD, showNotification, addEventLog, updateEventLogDisplay, renderInventory, updateAdminPlayersList, renderOnlinePlayersList } from '../ui/ui-controller.js';
import { initMap, updatePlayerPosition, getDistance, updateOtherPlayers, renderStaticMonsters, updateDebugCoords, centerOnPlayer, updateArenas } from '../map/map.js';
import { loadStaticMonsters, buildStaticMonsters } from '../gameplay/monsters.js';
import { initCharacterSelection } from '../ui/character-selection.js';
...
async function init() {
        ...
        if (!window._combatReconnectionChecked) {
            ...
            if (selfData && selfData.status === 'in_combat' && !gameState.combat) {
                ...
                if (combatId.startsWith('combat_')) {
                    import('../gameplay/combat.js').then(c => c.joinUnifiedCombat(combatId));
                    ...
                    import('../gameplay/combat.js').then(c => c.startPvPCombat(battleId));
    ...
    if (mapEl) {
        ...
        const { fetchSpawnedObjectsOnce, getCityZones, getTemplates } = await import('../firebase/firebase-service.js');
        ...
        if (gameState.debug.enabled) {
            const { loadTestPlayersToMap, isAdmin } = await import('../firebase/firebase-service.js');
            if (isAdmin()) {
...
async function initializeDebugMode() {
    ...
    if (gameState.debug.enabled) {
        ...
        // Update MP Debug UI
        import('../ui/ui-controller.js').then(({ refreshPlayersList, updateMultiplayerDebugUI, refreshSettingsVisibility }) => {
...
window.toggleDebugMode = function () {
    ...
    if (gameState.debug.enabled) {
        ...
        import('../firebase/firebase-service.js').then(({ loadTestPlayersToMap }) => {
            loadTestPlayersToMap();
        });
    } else {
