...
import { claimCastle } from "../firebase/firebase-service.js";
import { getCitadels, setCitadels } from "./territory-service.js";
...
export function initKingdom() {
  ...
  console.log(
    `👑 Kingdom System Initialized | ${citadels.length} citadels loaded`,
  );
...
export function checkCitadelProximity() {
  ...
  // Log visible territory stats periodically (every proximity check)
  const citadels = getCitadels();
  if (
    (citadels.length > 0 && !checkCitadelProximity._lastLog) ||
    Date.now() - checkCitadelProximity._lastLog > 30000
  ) {
    checkCitadelProximity._lastLog = Date.now();
    console.log(
      `📊 Territory Overview: ${citadels.length} citadels, ${gameState.currentDistrict?.name || "Unknown"} current zone`,
    );
  }
  ...
  const targetName = district.citadel
    ? district.citadel.name
    : district.name + " Citadel";
...
export function openCitadelMenu() {
  ...
  // Normalize data
  const citadelName = district.citadel
    ? district.citadel.name
    : district.name + " Citadel";
  ...
  document.getElementById("citadel-ruler").textContent =
    district.kingName || "None";
  ...
  if (isKing) {
    ...
    actionBtn.onclick = () =>
      showNotification("Tax management coming soon!", "info");
    actionBtn.className =
      "w-full py-3 bg-blue-600 rounded-lg font-bold hover:bg-blue-500";

    // Show Safe Storage button
    const storageBtn = document.createElement("button");
    storageBtn.className =
      "w-full mt-2 py-3 bg-green-700 rounded-lg font-bold hover:bg-green-600";
    ...
    actionBtn.className =
      "w-full py-3 bg-red-600 rounded-lg font-bold hover:bg-red-500 animate-pulse";
  } else {
    ...
    actionBtn.className =
      "w-full py-3 bg-yellow-600 rounded-lg font-bold hover:bg-yellow-500";
  }
...
function showChallengeDialog(district) {
  if (
    confirm(
      `Do you want to challenge ${district.kingName} for the throne?\n\nCost: 500 Gold\nWin Condition: PvP Duel`,
    )
  ) {
...
function initiateChallengeLevel(district) {
  ...
  showNotification(
    `⚔️ Challenge Request Sent to ${district.kingName}!`,
    "success",
  );
  addEventLog(
    `Challenged ${district.kingName} for control of ${district.name}`,
    "combat",
  );
...
async function claimThrone(district) {
  if (
    confirm(
      "Claim this unclaimed district? It will cost 1000 Gold to establish your rule.",
    )
  ) {
    ...
    // Persist to Firestore castles collection
    const citadelData = {
      name: district.name + " Citadel",
      lat: district.center?.lat || district.citadel?.lat,
      lng: district.center?.lng || district.citadel?.lng,
      ownerId: gameState.player.id || gameState.player.odooId,
      ownerName: gameState.player.name || "Unknown",
      ownerAvatar: gameState.player.avatar || "👤",
      powerMultiplier: 1,
      cityId: district.cityId || null,
      claimedAt: Date.now(),
    };

    const castleId = district.id || `citadel_${Date.now()}`;
    const success = await claimCastle(castleId, citadelData);

    if (success) {
      // Update local district state
      district.kingId = citadelData.ownerId;
      district.kingName = citadelData.ownerName;

      showNotification(
        `👑 Hail to the King! You now control ${district.name}!`,
        "success",
      );
      addEventLog(`Claimed the throne of ${district.name}`, "success");

      // Update territory-service cache
      const citadels = getCitadels();
      const existing = citadels.find((c) => c.id === castleId);
      if (existing) {
        existing.ownerId = citadelData.ownerId;
        existing.ownerName = citadelData.ownerName;
      } else {
        citadels.push({ id: castleId, ...citadelData });
        setCitadels(citadels);
      }

      console.log(`👑 Throne claimed and saved to Firestore: ${castleId}`);
    } else {
      // Refund gold on failure
      gameState.player.gold += 1000;
      showNotification("❌ Failed to claim throne. Gold refunded.", "error");
    }
