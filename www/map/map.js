...
export function createPlayerMarker(lat, lng, name, avatar, playerId, level = 1, isTestPlayer = false, userId = null, playerData = {}) {
    ...
    marker.on('click', async () => {
        ...
        if (currentStatus === 'in_combat') {
            import('../auth-ui/ui-controller.js').then(m => m.showNotification('⚔️ This player is in combat!', 'warning'));
            return;
        }
        ...
        if (dist > 50) {
            import('../auth-ui/ui-controller.js').then(m => m.showNotification(`❌ Too far! (${Math.round(dist)}m)`, 'warning'));
            return;
        }
