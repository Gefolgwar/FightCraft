...
let currentRecipe = {
    ...
    layers: {
        monsters: { templates: [{ templateId: "goblin", weight: 30 }, { templateId: "skeleton", weight: 20 }, { templateId: "wolf", weight: 25 }, { templateId: "orc", weight: 15 }, { templateId: "wraith", weight: 10 }] },
        shops: { templates: [{ templateId: "weapons_shop", weight: 40 }, { templateId: "armor_shop", weight: 30 }, { templateId: "potion_shop", weight: 30 }] },
        vaults: { templates: [{ templateId: "ancient_vault", weight: 50 }, { templateId: "dragon_vault", weight: 20 }, { templateId: "cursed_vault", weight: 30 }] },
        castles: { templates: [{ templateId: "stone_fortress", weight: 40 }, { templateId: "dark_citadel", weight: 25 }, { templateId: "royal_castle", weight: 35 }] },
    },
};
...
export async function initProceduralMap(containerId, options = {}) {
    ...
    map.fitBounds([[-60, -170], [70, 180]], { padding: [10, 10] });
...
export async function generateWorld() {
    ...
    const citiesWithBoundaries = cities.filter(c => cityBoundaries[c.id] && cityBoundaries[c.id].boundary);
    worldCitadels = generateAllCitadels(citiesWithBoundaries, currentRecipe, cityBoundaries, cityPOIs);
    ...
    const markers = worldCitadels.map(obj => {
        ...
        const marker = L.circleMarker([obj.lat, obj.lng], {
            radius: 6, fillColor: cc, color: cc, weight: 1, opacity: 0.9, fillOpacity: 0.7,
        });
        marker.bindPopup(() => `
    ...
    log(`✅ ${worldCitadels.length} citadels generated in ${genTime}ms for ${cities.length} cities`, "success");
    renderViewport();
}
...
async function loadDependencies() {
    ...
    try {
        ...
        cities = [{ id: "berlin", name: "Berlin", lat: 52.52, lng: 13.405, population: 3748148, country: "DE" }];
    }
    updateStats("cities", cities.length);
    ...
    try {
        ...
        if (bResp.ok) {
            ...
            const validCount = Object.keys(cityBoundaries).filter(k => cityBoundaries[k].boundary).length;
            totalBoundaryCount = validCount;
            updateStats("boundaries", `${validCount} / ${Object.keys(cityBoundaries).length}`);
            log(`Boundaries: ${validCount} valid entries loaded`, "success");
        }
    } catch (e) { log("No city boundaries file yet", "info"); }
    ...
    } catch (e) { log("No city POIs file yet", "info"); }
}
...
function renderViewport() {
    ...
    const boundsObj = { north: bounds.getNorth(), south: bounds.getSouth(), east: bounds.getEast(), west: bounds.getWest() };
    ...
    const lodName = zoom <= 7 ? "Citadels only" : zoom <= 10 ? "+Castles" : zoom <= 13 ? "+Shops/Vaults" : "Full (all types)";
    ...
    const t0 = performance.now();
    const objects = generateViewportObjects(boundsObj, currentRecipe, cities, zoom);
    ...
    const markers = objects.map(obj => {
        ...
        const marker = L.circleMarker([obj.lat, obj.lng], { radius: r, fillColor: color, color: color, weight: 1, opacity: 0.8, fillOpacity: 0.6 });
        marker.bindPopup(() => `
            <div style="font-family:system-ui;font-size:12px;">
    ...
    if (objects.length > 0) {
        log(`Viewport: ${objects.length} ${types.join("+")} in ${genTime}ms (zoom ${zoom})`, "success");
    }
    renderZones();
}
...
function renderZones() {
    ...
    const boundsObj = { north: bounds.getNorth(), south: bounds.getSouth(), east: bounds.getEast(), west: bounds.getWest() };
    const pad = 0.5;
    const visibleCities = cities.filter(c => c.lat >= boundsObj.south - pad && c.lat <= boundsObj.north + pad && c.lng >= boundsObj.west - pad && c.lng <= boundsObj.east + pad);
    if (visibleCities.length === 0 || visibleCities.length > 50) return;

    const t0 = performance.now();
    let totalZones = 0, totalCells = 0;
    ...
    for (const city of visibleCities) {
        ...
        let outerRing = Array.isArray(coords[0]) && Array.isArray(coords[0][0]) && typeof coords[0][0][0] === "number" ? coords[0] : (Array.isArray(coords[0]) && typeof coords[0][0] === "number" ? coords : null);
        ...
        if (!zoneCache.has(city.id)) {
            ...
            zoneCache.set(city.id, { zones: result.zones, geojson: zonesToGeoJSON(result.zones) });
        }

        const cached = zoneCache.get(city.id);
        if (cached.geojson.features.length > 0) {
            const geoLayer = L.geoJSON(cached.geojson, {
                style: feature => ({ color: feature.properties.color, weight: 1, fillOpacity: 0.2, fillColor: feature.properties.color, interactive: false })
            });
    ...
    if (totalZones > 0) {
        log(`Voronoi: ${totalZones} zones (${totalCells} cells) in ${(performance.now() - t0).toFixed(1)}ms`, "success");
    }
}
...
function renderBoundaries() {
    ...
    let visibleCities = zoom < 5 ? cities.filter(c => cityBoundaries[c.id] && cityBoundaries[c.id].boundary) : cities.filter(c =>
        c.lat >= boundsObj.south - pad && c.lat <= boundsObj.north + pad &&
    ...
    for (const city of visibleCities) {
        ...
        let geojsonCoords = Array.isArray(coords[0]) && Array.isArray(coords[0][0]) && typeof coords[0][0][0] === "number" ? [coords[0]] : (Array.isArray(coords[0]) && typeof coords[0][0] === "number" ? [coords] : null);
        if (!geojsonCoords) continue;

        const color = cityColor(city.name || city.id);
        if (zoom >= 8) {
            boundaryLayerGroup.addLayer(L.geoJSON({ type: "Feature", geometry: { type: "Polygon", coordinates: geojsonCoords }, properties: { cityId: city.id } }, {
                style: { color, weight: 2, fillOpacity: 0, dashArray: "8, 4", interactive: false }
            }));
        } else {
            boundaryLayerGroup.addLayer(L.circleMarker([city.lat, city.lng], { radius: 4, fillColor: color, color, weight: 1, opacity: 0.9, fillOpacity: 0.7, interactive: false }));
        }
    }
}
