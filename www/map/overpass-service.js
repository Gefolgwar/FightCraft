...
export class OverpassService {
  ...
  static async fetchJSON(query, attempt = 0) {
    if (attempt > 3)
      throw new Error("Overpass API failed after multiple retries.");
    ...
    try {
      ...
      if (resp.status === 429 || resp.status === 504 || resp.status === 502) {
        ...
        console.warn(
          `⚠️ Overpass Mirror ${endpoint} reported ${resp.status}. Rotating to ${nextEndpoint}...`,
        );
      ...
      if (text.trim().startsWith("<")) {
        throw new Error(
          "Overpass returned HTML instead of JSON (Server Overloaded)",
        );
      }
    ...
    try {
      ...
      const candidates = data.elements.filter(
        (e) =>
          ...
          e.tags &&
          e.tags.name &&
          e.tags.name.toLowerCase().includes(cityName.toLowerCase()),
      );

      // Sort by admin_level (4 = State/Capital, 8 = City, 9 = District). We want the lowest (largest area).
      const boundaryRel = candidates.sort(
        (a, b) =>
          parseInt(a.tags.admin_level || 10) -
          parseInt(b.tags.admin_level || 10),
      )[0];
      ...
      let population = null;
      if (boundaryRel) {
        if (boundaryRel.tags && boundaryRel.tags.population) {
          population = parseInt(
            boundaryRel.tags.population.replace(/\D/g, ""),
            10,
          );
        }
      ...
      const districts = includeDistricts
        ? await this.processDistrictData(data)
        : [];
      return { boundary, districts, boundaryId, population };
    } catch (e) {
            ...
            if (rings.length > 0) {
              features.push({
                ...
                geometry: {
                  ...
                  coordinates:
                    rings.length > 1 ? rings.map((r) => [r]) : [rings[0]],
                },
                properties: rel.tags || {},
              });
            }
  ...
  static async processDistrictData(data) {
    ...
    for (const element of data.elements) {
      if (element.type === "relation" && element.members) {
        const name =
          element.tags.name || element.tags["name:en"] || "Unknown District";
        let points = [];
