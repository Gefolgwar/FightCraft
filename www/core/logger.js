...
export function initLogger() {
  ...
  console.warn = (...args) => {
    // Filter out harmless Firebase WebChannel reconnect spam
    if (
      args.length > 0 &&
      typeof args[0] === "string" &&
      args[0].includes("WebChannelConnection")
    ) {
      return; // Silently ignore to prevent console flooding
    }
...
window.clearDebugConsole = function () {
  ...
  if (content) {
    content.innerHTML =
      '<div class="text-gray-500 italic">-- Console cleared --</div>';
  }
};
