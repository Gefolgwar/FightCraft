...
import {
  ...
  loadPlayerFromCloud,
  getCurrentUser,
  subscribeToPlayersRTDB,
  saveCharacter,
  getCharacter,
  logout,
  subscribeToSpawnedObjects,
  registerPlayerInRTDB,
  invalidateSpawnedObjectsCache,
...
async function init() {
  ...
  if (gameState.player) {
    ...
    if (gameState.player.statPoints === undefined)
  ...
  // Sync role from Firebase Auth into gameState so map.js filters work correctly.
  // Character documents don't store role — it lives on the user profile / userRole variable.
  if (!gameState.player.role) {
    gameState.player.role = isAdmin()
      ? "admin"
      : isModerator()
        ? "moderator"
        : "player";
  ...
  try {
    ...
    if (data.position && data.position.lat) {
      ...
      console.warn(
        "GPS unavailable, using default coordinates (Berlin):",
        error.message,
      );
  ...
  if (_pendingPvPReconnect && !gameState.combat) {
    console.log(
      "🔄 Reconnecting to PvP battle from localStorage:",
      _pendingPvPReconnect,
    );
    import("../gameplay/combat.js").then((c) => {
  ...
  if (mapEl) {
    ...
    const { fetchSpawnedObjectsOnce, getTemplates } =
      await import("../firebase/firebase-service.js");

    // Find closest city for initial sync
    let closestCityId = "berlin";
    ...
    console.log(
      `🌍 Bundling startup reads (Auto-load) for city: ${closestCityId}...`,
    );
    const [loadedObjects] = await Promise.all([
      fetchSpawnedObjectsOnce(),
      // getCityZones(closestCityId), // LEGACY: Disabled to save bandwidth
    ...
    if (loadedObjects && loadedObjects.length > 0) {
      ...
      import("../map/poi.js").then(({ addExternalPOIs, renderPOIs }) => {
        addExternalPOIs(freshPOIs);
        if (typeof renderPOIs === "function") renderPOIs();
      });
    }

    // Subscribe to world metadata changes (reacts to admin template toggles in real-time)
    subscribeToWorldMetadata(() => {
      console.log("🔄 World metadata changed — re-syncing map objects...");
      invalidateSpawnedObjectsCache();
      updateVisibility();
    });

    // Log active templates + per-template object breakdown
    try {
      const { getWorldSnapshots, isAdmin } =
        await import("../firebase/firebase-service.js");
      if (isAdmin()) {
        ...
        const nameMap = {};
        snapshots.forEach((s) => {
          nameMap[s.id] = s.name || s.id;
        });

        if (active.length > 0) {
          const names = active.map((s) => s.name || s.id).join(", ");
          addEventLog(
            `📋 Active templates (${active.length}): ${names}`,
            "system",
          );
          console.log(`📋 Active templates: ${names}`);
        } else {
          addEventLog("📋 No active templates", "warning");
        }

        if (loadedObjects && loadedObjects.length > 0) {
          const byTemplate = {};
          loadedObjects.forEach((o) => {
            const tid = o.sourceTemplateId || "_untagged_";
            if (!byTemplate[tid])
              byTemplate[tid] = {
                monsters: 0,
                shops: 0,
            ...
            byTemplate[tid].total++;
            if (o.type === "monster") byTemplate[tid].monsters++;
            else if (o.type === "shop") byTemplate[tid].shops++;
            else if (o.type === "castle") byTemplate[tid].castles++;
            else if (o.type === "vault") byTemplate[tid].vaults++;
            else byTemplate[tid].other++;
          });

          for (const [tid, c] of Object.entries(byTemplate)) {
            const tName =
              tid === "_untagged_" ? "Untagged" : nameMap[tid] || tid;
            const parts = [];
            if (c.monsters) parts.push(`${c.monsters} monsters`);
            if (c.shops) parts.push(`${c.shops} shops`);
            if (c.castles) parts.push(`${c.castles} castles`);
            if (c.vaults) parts.push(`${c.vaults} vaults`);
            if (c.other) parts.push(`${c.other} other`);
            addEventLog(
              `🗺️ ${tName}: ${parts.join(", ")} (${c.total} total)`,
              "system",
            );
          }
          ...
          active.forEach((s) => {
            if (!byTemplate[s.id]) {
              addEventLog(
                `⚠️ ${s.name || s.id}: ACTIVE but 0 objects on map`,
                "warning",
              );
            }
          });
          ...
          addEventLog(
            `🌍 Total on map: ${loadedObjects.length} objects`,
            "system",
          );

          // Auto-tag legacy untagged objects (one-time migration)
          if (byTemplate["_untagged_"] && active.length > 0) {
            addEventLog(
              `🔧 Tagging ${byTemplate["_untagged_"].total} legacy objects — one-time migration...`,
              "system",
            );
            import("../firebase/firebase-service.js")
              .then(({ tagLegacySpawnedObjects }) =>
                tagLegacySpawnedObjects(loadedObjects, active),
              )
              .then((count) => {
                if (count > 0) {
                  addEventLog(
                    `✅ Tagged ${count} objects. Deactivation will now work correctly.`,
                    "success",
                  );
                } else {
                  addEventLog(
                    `⚠️ Could not match objects to templates by coordinates.`,
                    "warning",
                  );
                }
              })
              .catch((e) => console.warn("Auto-tagging failed:", e));
          }
        } else {
          addEventLog("🌍 No objects loaded from database", "warning");
        }
      } else {
        // Non-admin: simple summary
        if (loadedObjects && loadedObjects.length > 0) {
          const mc = loadedObjects.filter((o) => o.type === "monster").length;
          addEventLog(
            `🌍 World loaded: ${mc} monsters, ${loadedObjects.length - mc} POIs`,
            "system",
          );
        }
      }
    } catch (e) {
      console.warn("Could not log template info:", e);
    }

    updateProgress("Scanning landmarks...", 90);
    const { checkAndFetchPOIs } = await import("../map/poi.js");
    await checkAndFetchPOIs();

    // Load test players to map immediately (if admin in debug mode)
    if (gameState.debug.enabled) {
      const { loadTestPlayersToMap, isAdmin } =
        await import("../firebase/firebase-service.js");
      if (isAdmin()) {
  ...
  // Start Loops
  import("../map/poi.js").then(({ processIncome }) => {
    setInterval(() => {
      updateRegeneration();
      processIncome();
...
window.startGameWithCharacter = async function (characterId, data) {
  ...
  // Find closest city for manual character load
  let closestCityId = "berlin";
  if (gameState.player && gameState.player.position) {
    let minD = Infinity;
    CITY_ANCHORS.forEach((c) => {
  ...
  console.log(`🌍 Bundling startup reads for city: ${closestCityId}...`);
  await Promise.all([
    fetchSpawnedObjectsOnce(),
    // getCityZones(closestCityId), // LEGACY: Disabled to save bandwidth
  ...
  if (mapEl) {
    ...
    try {
      ...
      if (isAdmin()) {
        ...
        if (cachedWorld && cachedWorld.length > 0) {
          ...
          addEventLog(
            `🌍 Total on map: ${cachedWorld.length} objects`,
            "system",
          );
          ...
          if (byTemplate["_untagged_"] && active.length > 0) {
            ...
            import("../firebase/firebase-service.js")
              .then(({ tagLegacySpawnedObjects }) =>
                tagLegacySpawnedObjects(cachedWorld, active),
              )
              .then((count) => {
                if (count > 0) {
                  addEventLog(
        ...
        if (cachedWorld && cachedWorld.length > 0) {
          const mc = cachedWorld.filter((o) => o.type === "monster").length;
          addEventLog(
            `🌍 World loaded: ${mc} monsters, ${cachedWorld.length - mc} POIs`,
            "system",
          );
        }
      }
    ...
    // Start World Sync (handles city changes during gameplay)
    setupWorldSync();
...
export async function saveGame() {
  ...
  const isTestMode =
    !!(window._controllingPlayer && window._controllingPlayer.id) ||
  ...
  if (isTestMode) {
    const testCharId =
      (window._controllingPlayer && window._controllingPlayer.id) ||
      (gameState.id && gameState.id !== window._currentCharacterId
        ? gameState.id
        : null);
...
window.teleportToCoords = function () {
  ...
  if (!isNaN(lat) && !isNaN(lng)) {
    ...
    showNotification(
      `✈️ Teleported to ${lat.toFixed(4)}, ${lng.toFixed(4)}`,
      "success",
    );
  } else {
...
async function initializeDebugMode() {
  ...
  if (gameState.debug.enabled) {
    ...
    // Update MP Debug UI
    import("../auth-ui/ui-controller.js").then(
      ({
        refreshPlayersList,
...
window.teleportToCoords = function () {
  ...
  updatePlayerPosition(lat, lng);
  showNotification(
    `📍 Teleported to ${lat.toFixed(4)}, ${lng.toFixed(4)}`,
    "success",
  );
};
...
window.resetGame = async function () {
  if (
    confirm(
      "Are you sure you want to reset all progress? This will reset your Level, Stats, Inventory and Gold!",
    )
  ) {
    ...
    // Clear equipment
    Object.keys(gameState.equipment).forEach(
      (slot) => (gameState.equipment[slot] = null),
    );
...
window.addXP = function (amount, source = "Debug") {
  gameState.player.xp =
    BigInt(gameState.player.xp) + BigInt(Math.floor(amount));
...
function levelUp() {
  ...
  gameState.player.xp =
    BigInt(gameState.player.xp) - BigInt(gameState.player.xpToNext);
  ...
  gameState.player.xpToNext = BigInt(
    500 * gameState.player.level * gameState.player.level,
  );
  ...
  if (window._cachedPlayersList && window.updateAdminPlayersList) {
    ...
    const selfInCache = window._cachedPlayersList.find(
      (p) => p.id === currentCharId || p.isSelf,
    );
    if (selfInCache) {
      console.log(
        `[INSTANT] ⚡ Multi-Admin updated: Lv.${selfInCache.level} -> Lv.${gameState.player.level}`,
      );
...
export function setupWorldSync() {
  ...
  _currentWorldSyncUnsub = subscribeToSpawnedObjects(closest.id, (entities) => {
    ...
    import("./gameState.js").then(({ setStaticMonsters }) => {
      ...
      import("../map/map.js").then(({ renderStaticMonsters }) => {
        renderStaticMonsters(true);
      });
    });

    // Sync Shops/Castles
    import("../map/poi.js").then(({ addExternalPOIs }) => {
