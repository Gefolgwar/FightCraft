...
// EntityConfigManager instance for shops
const configManager = new EntityConfigManager("shops", {
  templateType: "shop",
  accentColor: "blue",
...
const defaults = [
  {
    name: "Blacksmith",
    icon: "⚒️",
    osmTag: "shop=blacksmith",
    type: "shop",
    inventory: [{ id: "sword_basic", price: 100 }],
  },
  {
    name: "Potion Shop",
    icon: "🧪",
    osmTag: "shop=potion",
    type: "shop",
    inventory: [{ id: "potion_small", price: 50 }],
  },
  {
    name: "General Store",
    icon: "🏪",
    osmTag: "shop=general",
    type: "shop",
    inventory: [{ id: "ration_basic", price: 25 }],
  },
];
...
window.refreshSnapshotList = async () => {
  ...
  try {
    ...
    snaps.forEach((snap) => {
      ...
      const typeIcon =
        snap.type === "world" ? "🌍" : snap.type === "shop" ? "🏪" : "📦";
...
window.onSnapshotSelected = async () => {
  ...
  try {
    ...
    // Backward compatibility: if entityConfig.shops is empty,
    ...
    if (configManager.workingConfig.length === 0) {
      ...
      if (objects.length === 0 && snapshot?.seed) {
        ...
        try {
          ...
          logConsole(
            `ℹ️ Seed-based snapshot. Add shop templates below and set counts.`,
          );
        }
      }

      const shopObjects = objects.filter((o) => o.type === "shop");

      if (shopObjects.length > 0) {
        // Group by templateId and count
        const templateCounts = new Map();
        for (const obj of shopObjects) {
        ...
        logConsole(
          `ℹ️ Empty snapshot. Click templates in sidebar to add shop entries, then set counts.`,
        );
      }
    ...
    logConsole(
      `✅ Loaded ${configManager.workingConfig.length} shop config entries.`,
    );
    renderConfigTable();
    renderTemplateList();
  } catch (err) {
...
window.applyConfigChanges = async () => {
  ...
  try {
    ...
    if (result) {
      logConsole("✅ Shop config saved to snapshot!");
      renderConfigTable();
      renderTemplateList();
    } else {
