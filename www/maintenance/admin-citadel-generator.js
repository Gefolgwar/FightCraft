 ...
 */
export async function generateCitadelsAndZones(
  cityKey,
  capacity,
  templates,
  activeRulesMap = null,
  ...
  // Determine target capacity based on population if available
  let targetCapacity = capacity;
  if (ctx.population) {
    targetCapacity = Math.max(1, Math.ceil(ctx.population / 100000));
    console.log(
      `🌍 City Population: ${ctx.population} -> Target Citadels: ${targetCapacity}`,
    );
  } else {
    console.warn(
      `⚠️ Population not found for ${city.name}, falling back to requested capacity: ${targetCapacity}`,
    );
  }
  ...
  let queryFilters = "";
  const rulesToUse =
    activeRulesMap || new Map(templates.map((t) => [t.id, 100]));
  ...
  const citadelTemplate = templates.find(
    (t) => t.name.includes("Citadel") || t.icon === "🏯",
  );
  ...
  if (queryFilters) {
    ...
    try {
      ...
      console.error(
        "Overpass query failed, falling back to synthetic fill:",
        e,
      );
    }
  }
  ...
  if (cityBoundary) {
    processedCitadels = processedCitadels.filter((c) => {
      try {
        return turf.booleanPointInPolygon([c.lng, c.lat], cityBoundary);
      } catch (e) {
        return false;
      }
    });
  ...
  // 4. Synthetic Fill
  if (processedCitadels.length < targetCapacity) {
    let cityBounds = cityBoundary
      ? {
          minLng: turf.bbox(cityBoundary)[0],
          minLat: turf.bbox(cityBoundary)[1],
          maxLng: turf.bbox(cityBoundary)[2],
          maxLat: turf.bbox(cityBoundary)[3],
        }
      : {
          minLat: city.lat - 0.15,
          maxLat: city.lat + 0.15,
          minLng: city.lng - 0.25,
          maxLng: city.lng + 0.25,
        };
  ...
  if (processedCitadels.length > targetCapacity) {
    ...
    while (finalCitadels.length < targetCapacity && candidates.length > 0) {
  ...
  if (allCitadelsForZones.length >= 2) {
    zonesGeoJson = await generateCityTerritory(
      cityKey,
      allCitadelsForZones,
      null,
      cityBoundary,
  ...
  console.log(
    `🏰 Generated ${finalCitadels.length}/${targetCapacity} citadels for ${cityKey} (${finalCitadels.filter((c) => c.realWorldId).length} from OSM landmarks, ${finalCitadels.filter((c) => !c.realWorldId).length} synthetic)`,
  );
