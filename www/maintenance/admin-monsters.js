...
// EntityConfigManager instance for monsters
const configManager = new EntityConfigManager("monsters", {
...
window.onSnapshotSelected = async () => {
  ...
  try {
    ...
    // Backward compatibility: if entityConfig.monsters is empty,
    ...
    if (configManager.workingConfig.length === 0) {
      ...
      if (objects.length === 0 && snapshot?.seed) {
        ...
        try {
          ...
          // procedural engine may not export generateWorldFromSeed yet
          logConsole(
            `ℹ️ Seed-based snapshot. Add monster templates below and set counts.`,
          );
        }
      }

      const monsterObjects = objects.filter((o) => o.type === "monster");

      if (monsterObjects.length > 0) {
        // Group by templateId and count
        const templateCounts = new Map();
        for (const obj of monsterObjects) {
        ...
        logConsole(
          `🔄 Built config from ${monsterObjects.length} legacy monster objects (${templateCounts.size} templates).`,
        );
      } else if (objects.length === 0) {
        logConsole(
          `ℹ️ Empty snapshot. Click templates in sidebar to add monster entries, then set counts.`,
        );
      }
    ...
    logConsole(
      `✅ Loaded ${configManager.workingConfig.length} monster config entries.`,
    );
    renderConfigTable();
    renderTemplateList();
  } catch (err) {
...
window.applyConfigChanges = async () => {
  ...
  try {
    ...
    if (result) {
      logConsole("✅ Monster config saved to snapshot!");
      renderConfigTable();
      renderTemplateList();
    } else {
