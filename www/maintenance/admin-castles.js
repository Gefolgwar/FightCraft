...
// EntityConfigManager instance for castles
const configManager = new EntityConfigManager("castles", {
  templateType: "castle",
  accentColor: "blue",
...
async function loadTemplates() {
  ...
  // Ensure default templates if none exist
    ...
    el.innerHTML = `
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <div class="text-[10px] text-gray-500">Lv.${t.level || 1} • ${t.hp || 500} HP</div>
                    </div>
                </div>
...
window.openTemplateModal = () => {
  ...
  // Clear fields
  ["name", "icon", "osm", "level", "hp"].forEach(
    (id) => (document.getElementById(`tpl-${id}`).value = ""),
  );
  document.getElementById("tpl-level").value = 1;
  document.getElementById("tpl-hp").value = 500;
...
window.editTemplate = (id) => {
  ...
  document.getElementById("tpl-hp").value = t.hp || 500;
...
window.saveTemplateForm = async () => {
  const template = {
    ...
    hp: parseInt(document.getElementById("tpl-hp").value) || 500,
    type: "castle",
  };
...
window.refreshSnapshotList = async () => {
  ...
  try {
    ...
    snaps.forEach((snap) => {
      ...
      const typeIcon =
        snap.type === "world" ? "🌍" : snap.type === "castle" ? "🏰" : "📦";
...
window.onSnapshotSelected = async () => {
  ...
  try {
    ...
    // Backward compatibility: if entityConfig.castles is empty,
    ...
    if (configManager.workingConfig.length === 0) {
      ...
      const castleObjects = objects.filter((o) => o.type === "castle");

      if (castleObjects.length > 0) {
        // Group by templateId and count
        const templateCounts = new Map();
        for (const obj of castleObjects) {
        ...
        logConsole(
          `🔄 Built config from ${castleObjects.length} legacy castle objects (${templateCounts.size} templates).`,
        );
      } else if (objects.length === 0) {
        logConsole(
          `ℹ️ Empty snapshot. Click templates in sidebar to add castle entries, then set counts.`,
        );
      }
    ...
    logConsole(
      `✅ Loaded ${configManager.workingConfig.length} castle config entries.`,
    );
    renderConfigTable();
    renderTemplateList();
  } catch (err) {
...
window.applyConfigChanges = async () => {
  ...
  try {
    ...
    if (result) {
      logConsole("✅ Castle config saved to snapshot!");
      renderConfigTable();
      renderTemplateList();
    } else {
