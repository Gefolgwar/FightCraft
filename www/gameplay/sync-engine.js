...
export const SyncEngine = {
  ...
  async syncWorld(firestoreDb) {
    ...
    try {
      ...
      if (bundleUrl) {
        console.log(
          "📦 Bundle Available! Downloading from Storage...",
          bundleUrl,
        );
        try {
          const objects = await this.downloadBundle(bundleUrl);
          console.warn(
            `[WORLD] 🔍 Processing result: Array=${Array.isArray(objects)}, Count=${objects?.length}`,
          );

          if (objects && objects.length > 0) {
            console.warn(
              `[WORLD] ✅ SUCCESS. Saving ${objects.length} objects.`,
            );
          ...
          console.warn(
            "[WORLD] ❌ Bundle Load Failed. Falling back to Firestore.",
            err.message,
          );
        }
      }
      ...
      // Force FULL sync when serverHash is empty — this means the world was
      // invalidated (e.g. template deactivation cleared version_hash).
      // Delta sync cannot detect deleted objects, so a full replace is required.
      if (localTime === 0 || !serverHash) {
        console.warn(
          !serverHash
            ? "[WORLD] 🧹 Hash invalidated (template deactivation). Doing Full Firestore Sync."
            : "[WORLD] 🆕 NO CACHE. Doing Full Firestore Sync (Expensive!).",
        );
