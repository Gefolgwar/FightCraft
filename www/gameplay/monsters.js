...
// Procedural engine integration (H3-based deterministic monsters)
// These will be available after H3 is loaded
let _proceduralEngineLoaded = false;
let _generateMonstersForCell = null;
let _getMonstersForViewport = null;
let _getMonsterById = null;
...
const CITY_REGIONS = CITY_ANCHORS.map((anchor, idx) => {
  const radius =
    anchor.id === "berlin"
      ? 25
      : anchor.id === "london" || anchor.id === "paris" || anchor.id === "rome"
        ? 24
        : 20;
  return {
    id: anchor.id,
...
function getMonsterByLandUse(type, random) {
  ...
  return (
    MONSTER_LIBRARY.find((m) => m.templateId === templateId) ||
    MONSTER_LIBRARY[0]
  );
}
...
function pointInPolygon(lat, lng, polygon) {
  ...
  for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
    ...
    const xj = polygon[j][0],
      yj = polygon[j][1];
    const intersect =
      yi > lat !== yj > lat && lng < ((yj - yi) * (lat - xi)) / (xj - xi) + yi;
...
export function buildStaticMonsters() {
  console.log(
    "🚫 Local monster generation disabled. Relying on Database Sync.",
  );
  setStaticMonsters([]);
  return [];
}
...
export function saveStaticMonsters() {
  try {
    ...
    console.warn(
      "⚠️ Cannot save monsters to localStorage (quota exceeded). Data will be generated again on next launch.",
      err,
    );
  }
}
    ...
    try {
      ...
      if (!Array.isArray(monsters) || monsters.length === 0) {
        ...
        const isAdmin =
          ...
          window.gameState.player.role === "admin";
        const playerPos =
        ...
        if (!isAdmin && playerPos && window.turf) {
          ...
          filteredMonsters = monsters.filter((m) => {
            ...
            return (
              turf.distance(playerPoint, mPoint, { units: "kilometers" }) <= 100
            );
          });
        }
...
/**
 * Initialize the procedural monster engine
 * Called once after H3 library is loaded
 */
export async function initProceduralEngine() {
  try {
    const engine = await import("../core/procedural-engine.js");
    _generateMonstersForCell = engine.generateMonstersForCell;
    _getMonstersForViewport = engine.getMonstersForViewport;
    _getMonsterById = engine.getMonsterById;
    _proceduralEngineLoaded = true;
    console.log("✅ Procedural monster engine initialized");
  } catch (e) {
    console.error("Failed to load procedural engine:", e);
  }
}

/**
 * Get procedurally generated monsters for an array of H3 cells
 * @param {string[]} h3Cells - H3 cell indices
 * @param {Set<string>} defeatedIds - Set of defeated monster IDs to exclude
 * @returns {Array} Generated monster objects
 */
export function getProceduralMonsters(h3Cells, defeatedIds = new Set()) {
  if (!_proceduralEngineLoaded || !_getMonstersForViewport) {
    console.warn("Procedural engine not loaded yet");
    return [];
  }
  return _getMonstersForViewport(h3Cells, defeatedIds);
}

/**
 * Look up a single procedural monster by its deterministic ID
 * Used for combat reconnection after page refresh
 * @param {string} monsterId - e.g., 'proc_872a1070fffffff_3'
 * @returns {Object|null} Monster data
 */
export function lookupProceduralMonster(monsterId) {
  if (!_proceduralEngineLoaded || !_getMonsterById) return null;
  return _getMonsterById(monsterId);
}

/**
 * Check if a monster ID is from the procedural system
 */
export function isProceduralMonster(monsterId) {
  return typeof monsterId === "string" && monsterId.startsWith("proc_");
}
