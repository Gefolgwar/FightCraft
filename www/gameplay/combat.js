...
import { gameState, updatePlayer, getStaticMonsters, setStaticMonsters, STATIC_MONSTER_KEY, recalculateStats } from '../core/gameState.js';
import { ITEMS_DB, AFFIXES, GRID_SETTINGS } from './data.js';
import { showNotification, addEventLog, updateHUD } from '../auth-ui/ui-controller.js';
import { getDistance, renderStaticMonsters, isInsideArena } from '../map/map.js';
import { claimCastle, getCurrentUser, createArenaRTDB, removeArenaRTDB, updatePlayerStatus, updateSpawnedObject } from '../firebase/firebase-service.js';
...
export function startCombat(monster, isStatic = false) {
    ...
    import('../firebase/firebase-service.js').then(({ createArenaRTDB, updatePlayerStatus }) => {
    ...
    import('../map/map.js').then(m => m.renderArena(arenaId, arenaCenter, arenaRadius));
...
export async function startGroupCombat(monster, isStatic = false) {
    ...
    const { createUnifiedCombatRTDB, setGroupActiveCombatRTDB, createArenaRTDB } = await import('../firebase/firebase-service.js');
...
export async function joinUnifiedCombat(combatId) {
    ...
    const { subscribeToUnifiedCombat, updatePlayerStatus } = await import('../firebase/firebase-service.js');
    ...
    _unifiedCombatUnsub = subscribeToUnifiedCombat(combatId, (data) => {
        ...
        if (gameState.combat.arena) {
            import('../map/map.js').then(({ renderArena }) => {
...
async function resolveUnifiedRound(combatId, data) {
    ...
    const { updateUnifiedCombatRTDB } = await import('../firebase/firebase-service.js');
...
export async function executeAttack() {
    ...
    if (combat.isUnified) {
        const { submitUnifiedCombatMove } = await import('../firebase/firebase-service.js');
        const myCharId = window._currentCharacterId;
...
export function fleeCombat() {
    ...
    if (gameState.combat.monster?.isPlayer) {
        ...
        if (battleId) {
             import('../firebase/firebase-service.js').then(s => {
...
export function _cleanupCombatState() {
    ...
    if (arena) {
        ...
        import('../map/map.js').then(m => m.removeArenaFromMap(arena.id));
    }
...
export async function startPvPCombat(battleId) {
    ...
    // 1. Fetch Battle Data
    const { getBattleRequest, getCharacter, getCurrentUser } = await import('../firebase/firebase-service.js');
    ...
    const { getLivePlayer } = await import('../firebase/firebase-service.js');
    ...
    const { subscribeToPath } = await import('../firebase/firebase-service.js');
