...
│   ├── core/
│   │   ├── index.html       ← Main UI (true entry point)
│   │   ├── app.js           ← Entry point + init
│   │   ├── bridge.js        ← Window-global function registry
│   │   └── gameState.js     ← Game state management
│   ├── auth-ui/
│   │   ├── ui-controller.js ← UI logic + Online Players
│   │   └── character-selection.js ← Character switching
│   ├── firebase/
...
│   │   ├── groups.js        ← Group/party system
│   │   └── sync-engine.js   ← Optimization (IndexedDB)
│   ├── map/
│   │   ├── map.js           ← Leaflet map + markers
│   │   ├── districts.js     ← District system
│   │   ├── kingdom.js       ← Citadel capture
...
2. Update in `auth-ui/ui-controller.js`
3. Export to `window` via `core/bridge.js` if needed
4. Test in console

### Add New Menu Panel
```javascript
// 1. HTML (in core/index.html)
<div id="new-panel" class="fixed inset-0 z-[2000] hidden">
  <!-- content -->
</div>

// 2. auth-ui/ui-controller.js
const menuMap = {
